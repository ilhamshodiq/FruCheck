  ...
  try {
    ...
    reqDoc.get().then((buahDetail) => {
      if (!buahDetail.exists) {
        return res.status(404).send({message: 'Tidak ada buah yang ditemukan', error: true, message: "Buah tidak ditemukan" });
      }

      const response = buahDetail.data();
      return res.status(200).send({message : 'Data buah berhasil diambil', error: false, data: response });
    });
  } catch (error) {
    console.log(error);
